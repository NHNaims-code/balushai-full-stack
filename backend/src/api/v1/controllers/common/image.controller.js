...
const imageController = () => {
    return {
        ...
        uploadImage: async (req, res) => {
            ...
            const image_upload = await cloudinary.v2.uploader.upload(req.file?.path, { folder: 'balushai/product', use_filename: true });
        ...
        uploadDocument: async (req, res) => {
            ...
            const image_upload = await cloudinary.v2.uploader.upload(req.file?.path, { folder: 'balushai/documents', use_filename: true });
            if (!image_upload?.secure_url) return error().resourceError(res, 'Image Saved Failed . Please try again', 500);
        ...
        deleteImage: async (req, res) => {
            ...
            const response = await cloudinary.v2.uploader.destroy(`balushai/product/${public_id}`);
        ...
        deleteDocument: async (req, res) => {
            ...
            const response = await cloudinary.v2.uploader.destroy(`balushai/documents/${public_id}`);
            if (response?.result !== 'ok') return error().resourceError(res, 'Image Deleted Failed . Please try again', 500);
