...
export const getAllProducts = async (query) => {
    try {
        return await Campaign.findOne(query).lean().select('products -_id').populate('products');
    } catch (err) {
        console.log(err);
    }
};
...
export const getAllVendors = async (query) => {
    try {
        await Campaign.findOne(query).lean().select('vendors -_id').populate('vendors._id');
    } catch (err) {
        console.log(err);
    }
};
