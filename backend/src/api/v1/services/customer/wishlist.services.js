...
export const checkWishlistItemExist = async (user_id) => {
    try {
...
export const WishlistItemDelete = async (user_id, product_id) => {
    try {
        return await Wishlist.findOneAndUpdate({ user_id: user_id }, { $pull: { items: { product_id: item?.product_id, color_family: item?.color_family, size: item?.size } } }, { new: true }).lean()
            .populate('items.product_id', 'product_name -_id')
            .exec()
    } catch (err) {
        console.log(err);
