        ...
        case 'amount':
            return (
                <Typography variant='h6' sx={{ fontSize: '17px' }}>
                    Price: {total - fee} <br />
                    Shipping Fee: {fee}<br />
                    Balushai Discount: {Number(order?.balushai_voucher_discount) || 0}<br />
                    Seller Discount Total: {Number(order?.seller_discount) || 0}<br />
                    Total: {total -((Number(order?.balushai_voucher_discount) || 0) + Number(order?.seller_discount || 0)) }
                </Typography>
            )
        case 'transaction':
            return (
