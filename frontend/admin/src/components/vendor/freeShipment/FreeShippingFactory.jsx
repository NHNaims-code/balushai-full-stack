...
export default function VendorFreeShippingFactory(props) {
    ...
    const getAllFreeShippings = () => {
        TryCatch(async () => {
            if (props.type === 'all') {
                const res = await getFreeShipping(`promotions/vendor-all-free-shippings/${id}`);
                setFreeShipping(res?.data)
            } else if (props.type === 'active') {
                const res = await getFreeShipping(`promotions/vendor-active-free-shippings/${id}`);
                setFreeShipping(res?.data)
            } else if (props.type === 'deactive') {
                const res = await getFreeShipping(`promotions/vendor-deactive-free-shippings/${id}`);
                setFreeShipping(res?.data)
            } else {
                setFreeShipping([])
            }
            props?.getAllFreeShippingCountManage()
        });
    }
    useEffect(() => {
        getAllFreeShippings();
