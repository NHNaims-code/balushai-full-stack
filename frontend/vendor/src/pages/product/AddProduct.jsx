...
import { getAllNestedCategory } from "services/category/getAllCategory";
...
import { getVendorBrands } from "services/brand/getVendorBrands";
...
export default function AddProduct() {
  ...
  const [inputList, setInputList] = useState([{ color_family: "", image: [] }]);
  ...
  const handleAddMoreButton = () => {
    setInputList([...inputList, { color_family: "", image: [], },]);
  };
  ...
  const handleSizeInputChange = (value) => {
    ...
    inputList?.forEach((singleInputList, index) => {
      const { color_family, image } = singleInputList;
      let temp = [];
      if (color_family) {
        value?.forEach((val, index1) => {
          ...
          const temp1 = {
            ...
            seller_sku: rows[index][index1]?.seller_sku || "",
  ...
  const variantChange = (e, parentIndex, index) => {
    ...
    } else if (name === "seller_sku") {
      rows[parentIndex][index].seller_sku = value;
                  ...
                  >
                    <small>
                      ...
                      <ul>
                        ...
                        <li><b>
                          If Balushai missing your variant product properties please add this in <span className="text-danger">LONG DESCRIPTION</span>
                        </b></li>
                        <li><b>
                    ...
                    >
                      <div></div>
                      <Button variant="contained" onClick={handleAddMoreButton}>
                        Add More Variant
                      </Button>
                    </Box>
